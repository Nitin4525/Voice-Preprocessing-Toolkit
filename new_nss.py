# 改进谱减法的实现，
# 参考文献Berouti M , Schwartz R , Makhoul J .
# Enhancement of speech corrupted by acoustic noise[C]
# Acoustics, Speech, and Signal Processing,
# IEEE International Conference on ICASSP '79. IEEE, 1979.
...
def new_nss(wav, sr):
    ...
    # 估计噪声幅度谱，设前5帧为噪声，大约100ms(实际在语音段中只占了60ms，有帧叠)
    ...
    for n in range(framenum + 1):
        # 最后一帧果然会出问题，还是补零吧。。。
        wav_win = np.multiply(hamwin, wav[k:k + framesize])
        ...
        sub_speech = wav_win_mag ** expnt - \
            alpha * (noise_mean ** expnt)  # 这儿用的是功率谱
